 ...
 * Returns true if the source and export config checksums match, the entry was
 * recorded for the same output path, and the output file exists.
 * @param {string} [projectDir] - Base directory for resolving relative output paths
 */
export function isUpToDate(lockData, assetKey, variantKey, currentChecksum, outputPath, exportChecksum, projectDir = ".") {
  ...
  if (entry.outputPath !== outputPath) return false;
  if (!existsSync(resolve(projectDir, outputPath))) return false;
  return true;
}
