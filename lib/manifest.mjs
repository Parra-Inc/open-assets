...
/**
 * Get all export sizes from a collection (entries without a type field).
 */
export function flatSizes(collection) {
  return (collection.export || []).filter((e) => !e.type);
}

/**
 * Get all output actions from a collection (entries with a type field).
 */
export function flatOutputs(collection) {
  return (collection.export || []).filter((e) => e.type);
}
 ...
 * By default every configured export size is rendered; --width/--height or
 * --size narrow the selection.
 * @param {object} collection - The collection from the config
 * @param {{ width?: string|number, height?: string|number, size?: string, platform?: string }} opts
...
export function resolveRenderSizes(collection, opts = {}) {
  ...
  // Default: render at every configured export size
  return { sizes: flatSizes(collection) };
}
