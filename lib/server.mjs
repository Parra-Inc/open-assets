...
export async function startServer(projectDir, options = {}, renderer) {
  ...
  const { renderScreenshot, runXcodeOutput, closeBrowser, warmupBrowser } = renderer;
  ...
  // Pre-launch the shared browser in the background so the first interactive
  // render doesn't pay the ~2s launch cost. Failures surface on first render.
  if (typeof warmupBrowser === "function") {
    Promise.resolve(warmupBrowser()).catch(() => {});
  }
